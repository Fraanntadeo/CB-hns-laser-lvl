...
new const Mejoras[MEJORAS][][] =
{
	...
	// Descripcion
	{
		...
		"A tu multiplicador de exp se le suma x1, es decir que si^n\
		tengo x1, ahora tendria x2^n^n\
		\yADVERTENCIA:\w La exp que te saca, lo tendras que volver a^n\
...
new const Logros_GENERALES[LOGROS][][] =
{
	...
	// Descripcion del logro
	{
		...
		"Llegar a los 5.000.000 de EXP", // 9
		...
		"Apuesta en la loteria una cantidad de exp mayor^n\
...
public plugin_cfg()
{
	...
	/* Cargar levels.ini (configuracion de niveles) */
	...
	new level_cfg[128], key[64], sep[8], val[64]
	formatex(level_cfg, charsmax(level_cfg), "%s/levels.ini", cfg_dir)
	if (file_exists(level_cfg))
	{
		new f2 = fopen(level_cfg, "rt")
		if (f2 != 0)
		{
			while (!feof(f2))
			{
				fgets(f2, buffer, charsmax(buffer))
				if (!strlen(buffer) || buffer[0] == ';' || buffer[0] == '/' && buffer[1] == '/') continue
...
public mensajes()
{
	...
	g_query = SQL_PrepareQuery(g_hTuple, "SELECT nombre, nivel, rango, xp_normal FROM datos ORDER BY xp_normal DESC LIMIT 3")
	...
	if (SQL_Execute(g_query))
	{
		...
		while (SQL_MoreResults(g_query))
		{
			...
			ColorChat(0, GREEN, "%s^x01 Jugador^x04 #%d: %s^x01 Nivel:^x04 %s^x01 Rango:^x04 %s^x01 Exp:^x04 %s", szPrefix, count, nombre, addpoints(nivel), RANGOS[rango], addpoints(xp_normal))
...
public check(id)
{
	...
	// Ahora el progreso es por FRAGS (p_frags[][FRAGS_TOTAL])
	...
	switch (TEAM)
	{
		case LOGRO_TT:
		{
			...
			else
			{
				...
				{
					p_exp[id][EXP_NORMAL] += next_level(id, p_level[id])
					
					if (p_level[id] >= MAX_LEVEL(id)) p_level[id] = MAX_LEVEL(id)
					else p_level[id]++
				}
				
				p_exp[id][EXP_LEVEL] += str_to_num(exp)
				p_exp[id][EXP_NORMAL] += str_to_num(exp)
...
public task_TiempoEnEsconderse(id[1], tiempo)
{
	...
	if (tiempo == g_tiempo)
	{
		...
		else if (g_round_mod == MODO_DEAGLE)
		{
			...
			show_hudmessage(0, "MODO DEAGLE!^nMata a tus enemigos solo de HEADSHOT y suma el triple de EXP")
		...
		else if (g_round_mod == MODO_CUCHI)
		{
			...
			set_hudmessage(rojo, verde, azul, -1.0, 0.25, ef, 6.0, 10.0, 1.0, 2.0)
			show_hudmessage(0, "MODO CUCHI!^nCada frag que hagas ganas 5.678 de EXP")
...
public hook_say(id)
{
	...
	if (p_status[id] == STATUS_LOGED && is_user_playing(id))
	{
		...
		else if (equali(p_say, "/combo", 6))
		{
			...
			else ColorChat(id, GREEN, "%s^x01 Tu combo es de^x04 %s EXP", szPrefix, addpoints(p_party_info[id][5]))
		}
...
public REPITA_PASSWORD(id)
{
	...
	if (equal(arg, p_password[id]))
	{
		...
		formatex(szData[2], charsmax(szData) - 2, "%s", p_name[id])
		formatex(szText, charsmax(szText), "INSERT INTO cuentas (nombre, password, email, skype, pregunta, respuesta) VALUES ('%s', '%s', '%s', '%s', '%s', '%s')",
		p_name[id], p_password[id], p_email[id], p_skype[id], p_pregunta[id], p_respuesta[id])
		
		SQL_ThreadQuery(g_hTupleThread, "SQL_Crear", szText, szData, sizeof(szData))
	...
	if (SQL_Execute(g_query))
	{
		if (SQL_NumResults(g_query))
		{
			...
			if (equal(szPw, p_password[id]))
			{
				...
				formatex(query, charsmax(query), "UPDATE cuentas SET ip='%s' WHERE nombre COLLATE NOCASE LIKE ^"%s^"", newip, p_name[id])
...
public CAMBIAR_PASSWORD(id)
{
	...
	ColorChat(id, GREEN, "%s^x01 Tu nueva^x04 Contraseña^x01 es:^x04 %s", szPrefix, p_password[id])
	
	set_hudmessage(255, 255, 255, 0.025, 0.20, 0, 6.0, 6.0, 0.0, 1.0)
	ShowSyncHudMsg(id, g_SyncHud, "Tu contraseña se cambio exitosamente^nAhora tu contraseña es: %s", p_password[id])
}
...
public menu_principal(id)
{
	...
	formatex(szTitle, charsmax(szTitle), "%s v%s^n\dby \r%s^n\wTe faltan \y%s\w de EXP para el nivel \y%d", PLUGIN, VERSION, AUTHOR, addpoints(p_next_level(id)-p_exp[id][EXP_LEVEL]), p_level[id]+1)
...
public menu_mejoras_compra(id, item)
{
	...
	if (p_monedas[id] >= monedas && p_points[id] >= puntos && p_exp[id][EXP_NORMAL] >= exp && !p_mejoras[id][p_menu_mejoras[id]][COMPRADO])
...
public menu_logro_descrip(id, TEAM, key)
{
	...
	if (exp)
		len += formatex(szText[len], charsmax(szText) - len, "\r-\y %s\w Exp^n", addpoints(exp))
	if (puntos)
...
{
	g_query = SQL_PrepareQuery(g_hTuple, "SELECT nombre, nivel, rango, xp_normal FROM datos ORDER BY xp_normal DESC LIMIT 15")
...
public menu_loteria(id)
{
	...
	El ganador recibira la EXP que aposto multiplicada por 10, ejemplo:^n\
	Apostas 15.000 y ganas, tu recompenza sera de 150.000 EXP, En caso de^n\
	...
	if (p_apostado[id][0]) len += formatex(szText[len], charsmax(szText) - len, "\yEsta semana apostaste\w %s\r EXP\y al numero\w %d^n^n", addpoints(p_apostado[id][2]), p_apostado[id][1])
	
	len += formatex(szText[len], charsmax(szText) - len, "%s1. \wApostar a un numero^n", p_apostado[id][0] ? "\d" : "\r")
	
	len += formatex(szText[len], charsmax(szText) - len, "%s2. \wEXP a apostar^n", p_apostado[id][0] ? "\d" : "\r")
	
	if (!p_apostado[id][0]) len += formatex(szText[len], charsmax(szText) - len, "\r3. \wRealizar apuesta\d [NUM %d | EXP %s]^n", p_apostado[id][1], addpoints(p_apostado[id][2]))
...
public menu_loteria_handler(id, key)
{
	...
	else if (key == 1)
	{
		...
		else
		{
			...
			client_print(id, print_center, "Apostar EXP (Minimo 10.000)")
		}
	}
	else if (key == 2)
	{
		...
		else if (p_apostado[id][1] && p_apostado[id][2])
		{
			...
			formatex(query, charsmax(query), "UPDATE loteria SET num='%d', exp='%d' WHERE nombre COLLATE NOCASE LIKE ^"%s^"", p_apostado[id][1], p_apostado[id][2], p_name[id])
			SQL_ThreadQuery(g_hTupleThread, "SQL_Loteria", query, data, sizeof(data))
		}
		else
			client_print(id, print_center, "Selecciona un numero y cantidad de EXP a apostar")
	}
...
public SQL_Loteria(FailState, Handle:hQuery, szError[], iError, szData[], DataSize, Float:fQueueTime)
{
	...
	if (id > 0 && id < 33)
	{
		...
		ColorChat(0, GREEN, "%s^x01 El jugador^x04 %s^x01 Aposto^x04 %s EXP^x01 al numero^x04 %d", szPrefix, p_name[id], addpoints(p_apostado[id][2]), p_apostado[id][1])
		p_exp[id][EXP_NORMAL] -= p_apostado[id][2]
		p_exp[id][EXP_LEVEL] -= p_apostado[id][2]
		p_apostado[id][0] = 1
		apostadores++
	...
	else ColorChat(0, GREEN, "%s^x01 El pozo acumulado de la loteria es de^x04 %s EXP", szPrefix, addpoints(id))
}
...
public EXP_A_APOSTAR(id)
{
	...
	if (p_exp[id][EXP_NORMAL] < str_to_num(args))
	{
		client_print(id, print_center, "No tienes esa cantidad de EXP")
		return
	}
	
	p_apostado[id][2] = str_to_num(args)
	client_print(id, print_center, "Apostaras %s de EXP", addpoints(str_to_num(args)))
	ColorChat(id, GREEN, "%s^x01 Apostaras^x04 %s^x01 de^x04 EXP", szPrefix, addpoints(str_to_num(args)))
	menu_loteria(id)
}
	...
	if (numero < 0)
	{
		...
		if (ganancia)
		{
			for (id = 1; id <= g_MaxPlayers; id++)
			{
				...
				{
					p_exp[id][EXP_NORMAL] += exp*10
					p_exp[id][EXP_LEVEL] += exp*10
					check(id)
			...
			g_query = SQL_PrepareQuery(g_hTuple, "SELECT xp_normal, xp_level FROM datos WHERE nombre COLLATE NOCASE LIKE ^"%s^"", szName)
			...
			exp_n + (exp*10), exp_l + (exp*10), szName)
	...
	else if (numero == -5)
	{
		...
		if (ganancia)
		{
			for (id = 1; id <= g_MaxPlayers; id++)
			{
				...
				if (equali(p_name[id], szName))
				{
					p_exp[id][EXP_NORMAL] += (exp*10) + pozo
					p_exp[id][EXP_LEVEL] += (exp*10) + pozo
					check(id)
			...
			formatex(query, charsmax(query), "UPDATE datos SET xp_normal='%d', xp_level='%d' WHERE nombre COLLATE NOCASE LIKE ^"%s^"",
			exp_n + pozo + (exp*10), exp_l + pozo + (exp*10), szName)
			SQL_ThreadQuery(g_hTupleThread, "SQL_LoteriaSorteo", query)
			if (limpiar) Limpiar_loteria(1)
		}
	}
...
public Limpiar_loteria(pozo)
{
	...
	for (i = 0; i < ArraySize(g_datos_loteria); i++)
	{
		...
		formatex(query, charsmax(query), "UPDATE loteria SET num='0', exp='0' WHERE nombre COLLATE NOCASE LIKE ^"%s^"", nombre)
...
public suerte_exp(id)
{
	...
	else if (p_exp[id][EXP_NORMAL] >= 300) return ITEM_ENABLED
...
public menu_suerte_handler(id, menu, item)
{
	...
	switch (item)
	{
		case 0:
		{
			p_exp[id][EXP_NORMAL] -= 300
			p_exp[id][EXP_LEVEL] -= 300
			num = random_num(0, 100)
			p_suerte[id][0] = 1
			ColorChat(id, GREEN, "%s[SUERTE-EXP]^x01 Se te quito^x04 300 EXP^x01 para probar tu suerte", szPrefix)
			...
			switch (num)
			{
				...
				case 11..15:
				{
					ColorChat(id, GREEN, "%s[SUERTE-EXP]^x01 Ganaste^x04 El QUINTUPLE de EXP que te quito!", szPrefix)
					p_exp[id][EXP_NORMAL] += 300*5
					p_exp[id][EXP_LEVEL] += 300*5
					check(id)
				}
				case 16..20:
				{
					ColorChat(id, GREEN, "%s[SUERTE-EXP]^x01 Ganaste^x04 El DECUPLE de EXP que te quito!", szPrefix)
					p_exp[id][EXP_NORMAL] += 300*10
					p_exp[id][EXP_LEVEL] += 300*10
					check(id)
				}
				case 21..25:
				{
					ColorChat(id, GREEN, "%s[SUERTE-EXP]^x01 Ganaste^x04 7.000 de EXP!", szPrefix)
					p_exp[id][EXP_NORMAL] += 7000
					p_exp[id][EXP_LEVEL] += 7000
					check(id)
				}
				case 26..55:
				{
					ColorChat(id, GREEN, "%s[SUERTE-EXP]^x01 Perdiste^x04 1.000 :(", szPrefix)
					p_exp[id][EXP_NORMAL] -= 1000
					p_exp[id][EXP_LEVEL] -= 1000
				}
		...
		case 1:
		{
			...
			p_suerte[id][1] = 1
			ColorChat(id, GREEN, "%s[SUERTE-PUNTOS]^x01 Se te quito^x04 UN PUNTO^x01 para probar tu suerte", szPrefix)
			...
			switch (num)
			{
				...
				case 24..36:
				{
					switch (random_num(1, 2))
					{
						case 1:
						{
							ColorChat(id, GREEN, "%s[SUERTE-PUNTOS]^x01 Ganaste^x04 5.000 de EXP!!", szPrefix)
							p_exp[id][EXP_NORMAL] += 5000
							p_exp[id][EXP_LEVEL] += 5000
							check(id)
						}
						case 2:
						{
			...
			switch (num)
			{
				...
				case 51..60:
				{
					switch (random_num(1, 2))
					{
						case 1:
						{
							ColorChat(id, GREEN, "%s[SUERTE-MONEDAS]^x01 Ganaste^x04 10.000 de EXP!!", szPrefix)
							p_exp[id][EXP_NORMAL] += 10000
							p_exp[id][EXP_LEVEL] += 10000
							check(id)
						}
						case 2:
						{
...
public menu_estadisticas(id, pag)
{
	...
	{
		len += formatex(menu[len], charsmax(menu) - len, "\wExperiencia:\y %s^n", addpoints(p_exp[id][EXP_NORMAL]))
...
public menu_cuentas_ban_op_handler(id, menu, item)
{
	...
	switch (item)
	{
		case 0:
		{
			...
			formatex(query, charsmax(query), "UPDATE cuentas SET ban='' WHERE nombre COLLATE NOCASE LIKE ^"%s^"", nombre)
...
public NUEVA_FECHA(id)
{
	...
	formatex(query, charsmax(query), "UPDATE cuentas SET ban='%s' WHERE nombre COLLATE NOCASE LIKE ^"%s^"", args, p_menu_desbanear[id])
	SQL_ThreadQuery(g_hTupleThread, "SQL_ModificarBanCuenta", query, data, 1)
		...
		case MENU_PLAYERS_NIVELES:
		{
			if (!p_menu_admin[id][0])
			{
				for (i = 0; i < str_to_num(cantidad); i++)
				{
					...
					p_exp[p_jugador_seleccionado[id]][EXP_NORMAL] -= next_level(p_jugador_seleccionado[id], p_level[p_jugador_seleccionado[id]])
					
					if (p_exp[p_jugador_seleccionado[id]][EXP_LEVEL] > 0) p_exp[p_jugador_seleccionado[id]][EXP_LEVEL] = 0
				}
			}
			else if (p_menu_admin[id][0])
			{
...
public ShowHud(taskid)
{
	...
	else if (!p_hud[id][HUD_DESAC])
	{
		...
		if (p_hud[id][HUD_MIN])
		{
			...
			ShowSyncHudMsg(id, g_SyncHud2, "Clase: %s - Nivel: %d - Rango: %s - Experiencia: %s (%0.2f%%)", p_class_name[id], p_level[id], RANGOS[p_rango[id]],
			addpoints(p_exp[id][EXP_NORMAL]), float(p_exp[id][EXP_LEVEL] * 100) / float(p_next_level(id)))
		}
...
public RoundRestart()
{
	...
	for (id = 1; id <= g_MaxPlayers; id++)
	{
		...
		if (p_exp[id][EXP_NORMAL] >= 5000000) checkear_logro(id, LOGRO_GENERAL, 9)
		...
		if (g_round_mod == MODO_DEAGLE || g_round_mod == MODO_NORMAL)
		{
			if (is_user_in_party(id))
			{
				...
				{
					p_exp[id][EXP_NORMAL] += p_party_info[id][5]
					p_exp[id][EXP_LEVEL] += p_party_info[id][5]
					ColorChat(id, GREEN, "%s^x01 Recibiste tu combo de^x04 %s EXP", szPrefix, addpoints(p_party_info[id][5]))
					check(id)
...
public RoundStart_FT()
{
	...
	for (id = 1; id <= g_MaxPlayers; id++)
	{
		...
		else if (g_round_mod == MODO_NORMAL)
		{
			...
			{
				fm_set_user_health(id, ArrayGetCell(g_class_health, p_class[id]) + (p_hab[id][HAB_CT][HAB_CT_VIDA] * 3))
				p_round_vida[id] = ArrayGetCell(g_class_health, p_class[id]) + (p_hab[id][HAB_CT][HAB_CT_VIDA] * 3)
			...
			{
				fm_set_user_health(id, ArrayGetCell(g_class_health, p_class[id]) + (p_hab[id][HAB_TT][HAB_TT_VIDA] * 3))
				p_round_vida[id] = ArrayGetCell(g_class_health, p_class[id]) + (p_hab[id][HAB_TT][HAB_TT_VIDA] * 3)
				
				if (ArrayGetCell(g_class_armor, p_class[id]) || p_hab[id][HAB_TT][HAB_TT_CHALECO])
					cs_set_user_armor(id, ArrayGetCell(g_class_armor, p_class[id]) + (p_hab[id][HAB_TT][HAB_TT_CHALECO] * 3), CS_ARMOR_VESTHELM)
			}
		}
		...
		else if (termino)
		{
			...
			for (i = 1; i <= g_MaxPlayers; i++)
			{
				...
				else if (cs_get_user_team(i) == TWin)
				{
					p_points[i] += 2
					p_monedas[i] += 2
					p_exp[i][EXP_LEVEL] += 10000
					p_exp[i][EXP_NORMAL] += 10000
					ColorChat(i, GREEN, "%s^x01 Ganaste^x04 2^x03 Puntos^x01,^x04 2^x03 Monedas^x01 y^x04 10.000^x03 EXP", szPrefix)
					check(i)
				}
			...
			if (button&IN_ATTACK2 && !(oldbutton&IN_ATTACK2))
			{
				if (get_user_weapon(id) == CSW_DEAGLE && p_mejoras[id][5][HABILITADO])
				{
					if (p_poder_deagle[id])
					{
						...
						if (!is_user_in_party(id))
						{
							p_exp[id][EXP_LEVEL] += (247*3) * ((g_mult * p_mult[id]) + p_round_mult[id] + p_mejoras[id][4][HABILITADO])
							p_exp[id][EXP_NORMAL] += (247*3) * ((g_mult * p_mult[id]) + p_round_mult[id] + p_mejoras[id][4][HABILITADO])
						}
...
public fw_PlayerSpawn(id)
{
	...
	if (p_class_next[id] >= 0)
	{
		...
		ExecuteForward(g_fwClassLaser, ret, id, ArrayGetCell(g_class_lasers, p_class[id]))
...
public fw_Killed(victim, attacker)
{
	...
	else if (g_round_mod == MODO_DEAGLE)
	{
		if (!is_user_in_party(attacker))
		{
			p_exp[attacker][EXP_LEVEL] += (247*3) * ((g_mult * p_mult[attacker]) + p_round_mult[attacker] + p_mejoras[attacker][4][HABILITADO])
			p_exp[attacker][EXP_NORMAL] += (247*3) * ((g_mult * p_mult[attacker]) + p_round_mult[attacker] + p_mejoras[attacker][4][HABILITADO])
		}
		
		else if (is_user_in_party(attacker))
			set_party_exp(get_party_id(attacker), (247*3) * ((g_mult * p_mult[attacker]) + p_round_mult[attacker] + p_mejoras[attacker][4][HABILITADO]))
	...
	else if (g_round_mod == MODO_CUCHI)
	{
		if (!is_user_in_party(attacker))
		{
			p_exp[attacker][EXP_LEVEL] += 5678 * ((g_mult * p_mult[attacker]) + p_round_mult[attacker] + p_mejoras[attacker][4][HABILITADO])
			p_exp[attacker][EXP_NORMAL] += 5678 * ((g_mult * p_mult[attacker]) + p_round_mult[attacker] + p_mejoras[attacker][4][HABILITADO])
		}
		
		else if (is_user_in_party(attacker))
			set_party_exp(get_party_id(attacker), 5678 * ((g_mult * p_mult[attacker]) + p_round_mult[attacker] + p_mejoras[attacker][4][HABILITADO]))
	...
	else if (g_round_mod == MODO_LIDER)
	{
		if (!is_user_in_party(attacker))
		{
			p_exp[attacker][EXP_LEVEL] += 5500 * ((g_mult * p_mult[attacker]) + p_round_mult[attacker] + p_mejoras[attacker][4][HABILITADO])
			p_exp[attacker][EXP_NORMAL] += 5500 * ((g_mult * p_mult[attacker]) + p_round_mult[attacker] + p_mejoras[attacker][4][HABILITADO])
		}
		
		else if (is_user_in_party(attacker))
			set_party_exp(get_party_id(attacker), 5500 * ((g_mult * p_mult[attacker]) + p_round_mult[attacker] + p_mejoras[attacker][4][HABILITADO]))
		...
		if (matar)
		{
			...
			for (id = 1; id <= g_MaxPlayers; id++)
			{
				...
				else if (cs_get_user_team(id) == TWin)
				{
					...
					p_exp[id][EXP_LEVEL] += 10000
					p_exp[id][EXP_NORMAL] += 10000
					ColorChat(id, GREEN, "%s^x01 Ganaste^x04 2^x03 Puntos^x01,^x04 2^x03 Monedas^x01 y^x04 10.000^x03 EXP", szPrefix)
				}
			}
			ColorChat(0, TWin == CS_TEAM_CT ? BLUE : RED, "^x04%s^x01 Los sobrevivientes^x04 %s^x01 Ganaron:", szPrefix, TWin == CS_TEAM_CT ? "CT" : "TT")
			ColorChat(0, TWin == CS_TEAM_CT ? BLUE : RED, "^x04%s^x03 2^x01 Puntos,^x03 2^x01 Monedas y^x03 10.000^x01 de EXP!", szPrefix)
	...
	if (!is_user_in_party(attacker))
	{
		p_exp[attacker][EXP_LEVEL] += 247 * ((g_mult * p_mult[attacker]) + p_round_mult[attacker] + p_mejoras[attacker][4][HABILITADO])
		p_exp[attacker][EXP_NORMAL] += 247 * ((g_mult * p_mult[attacker]) + p_round_mult[attacker] + p_mejoras[attacker][4][HABILITADO])
	}
	
	else if (is_user_in_party(attacker))
		set_party_exp(get_party_id(attacker), 247 * ((g_mult * p_mult[attacker]) + p_round_mult[attacker] + p_mejoras[attacker][4][HABILITADO]))
...
public Guardar(id)
{
	...
	p_password[id], p_email[id], p_skype[id], p_pregunta[id], p_respuesta[id], p_name[id])
	...
	p_level[id], p_rango[id], p_class[id], p_points[id], p_frags[id][FRAGS_TOTAL], 0, p_plata[id], p_monedas[id], p_name[id])
	...
	p_damage[id][DAMAGE_HECHO], p_damage[id][DAMAGE_RECIBIDO], p_frags[id][FRAGS_TOTAL], p_frags[id][FRAGS_RECIBIDOS], p_frags[id][FRAGS_CARNAGE], p_frags[id][FRAGS_KNIFE], p_frags[id][FRAGS_WEAPON], p_frags[id][FRAGS_LASER], p_name[id])
	...
	p_hud[id][HUD_RED], p_hud[id][HUD_GREEN], p_hud[id][HUD_BLUE], p_hud[id][HUD_EFFECT], p_hud[id][HUD_MIN], p_hud[id][HUD_AB], p_hudx[id], p_hudy[id], p_name[id])
	...
	p_hab[id][HAB_CARNAGE][HAB_CAR_RECOIL], p_hab[id][HAB_CARNAGE][HAB_CAR_VELOCIDAD], p_hab[id][HAB_CARNAGE][HAB_CAR_VEL_DISPARO], p_hab[id][HAB_CARNAGE][HAB_CAR_RESISTENCIA], p_name[id])
	...
	p_logros_tt[id][4], p_logros_tt[id][5], p_logros_tt[id][6], p_logros_tt[id][7], p_name[id])
	...
	p_logros_ct[id][4], p_logros_ct[id][5], p_logros_ct[id][6], p_logros_ct[id][7], p_name[id])
	...
	p_logros_generales[id][10], p_logros_generales[id][11], p_logros_generales[id][12], p_logros_generales[id][13], p_logros_generales[id][14], p_name[id])
	...
	p_logros_generales[id][21], p_logros_generales[id][22], p_logros_generales[id][23], p_logros_generales[id][24], p_logros_generales[id][25], p_logros_generales[id][26], p_name[id])
	...
	p_mejoras[id][4][HABILITADO], p_mejoras[id][5][HABILITADO], p_mejoras[id][6][HABILITADO], p_name[id])
...
public CuentasBaneadas(FailState, Handle:hQuery, szError[], Error, szData[], DataSize, Float:fQueueTime)
{
	...
	while (SQL_MoreResults(hQuery))
	{
		...
		formatex(query, 554, "UPDATE cuentas SET ban='' WHERE nombre=^"%s^"", nombre)
...
public SQL_Crear(FailState, Handle:hQuery, szError[], Error, szData[], DataSize, Float:fQueueTime)
{
	...
	{
		szData[1]++
		formatex(szText, charsmax(szText), "INSERT INTO datos (nombre) VALUES ('%s')", szName)
	...
	{
		szData[1]++
		formatex(szText, charsmax(szText), "INSERT INTO estadisticas (nombre) VALUES ('%s')", szName)
	...
	else if (szData[1] == 2)
	{
		...
		szName, p_hud[id][HUD_RED], p_hud[id][HUD_GREEN], p_hud[id][HUD_BLUE], p_hud[id][HUD_EFFECT], p_hud[id][HUD_MIN], p_hud[id][HUD_AB], p_hudx[id], p_hudy[id])
	...
	{
		szData[1]++
		formatex(szText, charsmax(szText), "INSERT INTO habilidades (nombre) VALUES ('%s')", szName)
	...
	{
		szData[1]++
		formatex(szText, charsmax(szText), "INSERT INTO logros_tt (nombre) VALUES ('%s')", szName)
	...
	{
		szData[1]++
		formatex(szText, charsmax(szText), "INSERT INTO logros_ct (nombre) VALUES ('%s')", szName)
	...
	{
		szData[1]++
		formatex(szText, charsmax(szText), "INSERT INTO logros_gen (nombre) VALUES ('%s')", szName)
	...
	{
		szData[1]++
		formatex(szText, charsmax(szText), "INSERT INTO mejoras (nombre) VALUES ('%s')", szName)
	...
	{
		szData[1]++
		formatex(szText, charsmax(szText), "INSERT INTO loteria (nombre) VALUES ('%s')", szName)
	...
	else if (szData[1] == 10)
	{
		...
		{
			set_hudmessage(255, 255, 255, 0.025, 0.20, 0, 6.0, 6.0, 0.0, 1.0)
			ShowSyncHudMsg(id, g_SyncHud, "Registro exitoso!^nTu nombre es: %s^nTu contraseña es: %s", p_name[id], p_password[id])
			ColorChat(id, GREEN, "%s^x01 Registro exitoso! Tu nombre:^x04 %s^x01 Tu contraseña:^x04 %s", szPrefix, p_name[id], p_password[id])
			p_status[id] = STATUS_LOGED
			engclient_cmd(id, "jointeam", "5")
			engclient_cmd(id, "joinclass", "5")
			set_task(0.1, "menu_principal", id)
...
stock PartyDestroy(party_id, Bug = 0)
{
	...
	for (id = 1; id <= g_MaxPlayers; id++)
	{
		...
		if (get_party_id(id) == party_id)
		{
			...
			{
				ColorChat(id, GREEN, "%s^x01 Recibiste el combo de^x04 %s EXP", szPrefix, addpoints(p_party_info[id][5]))
				p_exp[id][EXP_NORMAL] += p_party_info[id][5]
				p_exp[id][EXP_LEVEL] += p_party_info[id][5]
				check(id)
			}
