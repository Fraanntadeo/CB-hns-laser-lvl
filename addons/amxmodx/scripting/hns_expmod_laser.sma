...
#define PLUGIN "LaserMines HNS + LVL"
#define VERSION "2.3"
...
new const
	CHATTAG[] 			= "!g[SERVER]!y",
	STR_NOTACTIVE[] 	= "Lasermines no se encuentran activos.",
	STR_DONTHAVEMINE[]	= "Usted no tiene lasermine.",
	STR_MAXDEPLOY[]		= "Maximo de lasermine han sido puestos.",
	STR_MANYPPL[]		= "Su equipo ya ha puesto el suficiente de lasermine...",
	STR_PLANTWALL[]		= "Usted debe plantar la lasermine en una pared!",
	
	STR_CBT[]			= "Su equipo no puede comprar y poner lasermine!",
	STR_CANTBUY[]		= "No se puede comprar en este servidor.",
	STR_HAVEMAX[]		= "Usted tiene el maximo de lasermine.",
	STR_NOMONEY[]		= "Usted no tiene el dinero suficiente para comprar un lasermine! ($",
	STR_NEEDED[]		= "necesita)",
	STR_DELAY[]			= "Usted puede comprar y poner lasermine despues de",
	STR_SECONDS[]		= "segundos.",
	STR_BOUGHT[]		= "Has comprado un lasermine.",
	STR_STATE[]			= "Lasers:",
	STR_NOACCESS[]		= "No tienes acceso, a este comando.",
	STR_CARNAGE[]		= "No puedes poner minas en^x04 carnage",
	STR_DONTPLANT[]		= "No puedes poner minas despues de terminada la ronda"
...
public plugin_init()
{
	...
	g_LDMG		= register_cvar("amx_ltm_dmg","999") 		//laser hit dmg
	g_LHEALTH	= register_cvar("amx_ltm_health","500")
	g_LTMAX		= register_cvar("amx_ltm_teammax","44")
	...
	g_LDELAY	= register_cvar("amx_ltm_delay","8")
	...
	register_event("TextMsg", "newround", "a", "2&#Game_C", "2&#Game_w")
...
public plugin_cfg()
{
	...
	server_cmd("exec %d/laser_cfg.cfg", cfg_dir)
...
{
	if (get_pcvar_num(g_LCMDMODE) != 0 && !fmod_is_carnage())
	{
		if (g_Plantar)
		...
		{
			ChatColor(id, "%s %s", CHATTAG, STR_DONTPLANT)
			return PLUGIN_HANDLED
		}
	}
	else if (fmod_is_carnage())
	{
		ChatColor(id, "%s %s", CHATTAG, STR_CARNAGE)
		return PLUGIN_HANDLED
	}
	return PLUGIN_HANDLED
}
...
public fmod_class_lasers(id, lasers)
{
	p_lasers[id] = lasers
}
...
bool:CanCheck(id,mode)
{
	...
	{
		ChatColor( id, "^x04%s^x01 %s",CHATTAG,STR_NOTACTIVE)
		return false
	}
	if( get_pcvar_num(g_LACCESS) != 0)
		if(!(get_user_flags(id) & ADMIN_IMMUNITY))
		{
			ChatColor( id, "^x04%s^x01 %s",CHATTAG,STR_NOACCESS)
			return false
		}
	if(!pev_user_alive(id)) return false
	if (!CheckCanTeam(id))
	{
		ChatColor( id, "%s %s",CHATTAG,STR_CBT)
		return false
	}
	if( mode == 0)
	{
		if(g_havemine[id] <= 0)
		{
			ChatColor( id, "%s %s",CHATTAG,STR_DONTHAVEMINE)
			return false
		}
	}
	if (mode == 1)
	{
		...
		if (g_havemine[id] >= p_lasers[id])
		{
			ChatColor( id, "%s %s",CHATTAG,STR_HAVEMAX)
			return false
		}
		if (fmod_have_money(id, 1) < 1)
		{
			ChatColor( id, "%s %s%d %s",CHATTAG, STR_NOMONEY, 1,STR_NEEDED)
			return false
		}
	}
	if(!CheckTime(id))
	{
		ChatColor(id, "%s %s %d %s",CHATTAG,STR_DELAY,get_pcvar_num(g_LDELAY)-g_nowtime,STR_SECONDS)
		return false
	}
...
bool:ReturnCheck( id )
{
	...
	if(g_havemine[id] + 1 > p_lasers[id]) return false
	new tgt,body,Float:vo[3],Float:to[3]
	get_user_aiming(id,tgt,body)
...
bool:CreateCheck( id )
{
	...
	{
		ChatColor( id, "%s %s",CHATTAG,STR_MANYPPL)
		return false
	}
	...
	ChatColor( id, "%s %s",CHATTAG,STR_PLANTWALL)
	DeleteTask(id)
...
CreateDamage(iCurrent,Float:DmgMAX,Float:Radius)
{
	...
	while((ent = engfunc(EngFunc_FindEntityInSphere, ent, vecSrc, Radius)) != 0)
	{
		...
		if (flFraction >= 0.9 || get_tr2(trRes, TR_pHit) == ent)
		{
			...
			if(iHitHP <= 0)
			{
				if(iHitTeam != _:CS_TEAM_SPECTATOR && iHitTeam != _:CS_TEAM_UNASSIGNED && iHitTeam != TeamID)
				{
					//cs_set_user_money(AtkID,cs_get_user_money(AtkID) + get_pcvar_num(g_LFMONEY))
...
CreateLaserDamage(iCurrent,isHit)
{
	...
	if((pev(isHit, pev_flags) & (FL_CLIENT | FL_FAKECLIENT | FL_MONSTER)))
	{
		...
		}else if(iHitTeam != _:CS_TEAM_SPECTATOR && iHitTeam != _:CS_TEAM_UNASSIGNED && iHitTeam != teamid)
		{
			emit_sound(isHit, CHAN_WEAPON, ENT_SOUND4, 1.0, ATTN_NORM, 0, PITCH_NORM )
...
{	
	...
	fmod_have_money(id, 1, 1)
	g_havemine[id]++;
	ChatColor( id, "%s %s",CHATTAG,STR_BOUGHT)
	emit_sound(id, CHAN_ITEM, ENT_SOUND5, VOL_NORM, ATTN_NORM, 0, PITCH_NORM)
	ShowAmmo(id)
	return PLUGIN_HANDLED
}
...
{ 
    new ammo[51] 
    formatex(ammo, 50, "%s %i/%i",STR_STATE, g_havemine[id],p_lasers[id])
...
public say_lasermine(id){
	...
	read_argv(1,said,31);
	if( !get_pcvar_num( g_LENABLE )  ){
//		ChatColor( id, "%s Lasermines are not currently active.",CHATTAG)
		return PLUGIN_CONTINUE
	}
	
	if (equali(said,"/buy lasermine") || equali(said,"/lm")) BuyLasermine(id)
	if (equali(said,"/unstuck") || equali(said,"/destrabar") || equali(said,"/desbugear"))
		desbugear(id)
...
public fw_Touch(ent, id)
{
	...
	static classname[55]; pev(ent, pev_classname, classname, 54)
	
	if (id >= 1 && id <= 32 && equali(classname, ENT_CLASS_NAME))
	{
		if (g_NoBug[id] && pev_valid(ent))
		{
			if (g_havemine[pev(ent, LASERMINE_OWNER)] < p_lasers[pev(ent, LASERMINE_OWNER)])
			{
				g_havemine[pev(ent, LASERMINE_OWNER)]++
				g_deployed[pev(ent, LASERMINE_OWNER)]--
				ChatColor(pev(ent, LASERMINE_OWNER), "%s Se te devolvio un laser que estaba bugeando a un jugador", CHATTAG)
			}
			ShowAmmo(pev(ent, LASERMINE_OWNER))
			g_NoBug[id] = false
...
{
	static ent, body, szClassName[33], szName[33]//, user[555]
	...
	{
		get_user_name(pev(ent, LASERMINE_OWNER), szName, 32)
		set_hudmessage(50, 50, 255, -1.0, 0.8, 0, 6.0, 0.1, 1.0, 1.0)
		show_hudmessage(id, "Creador del laser: %s", szName)
	}
}

resetspeed(id)
{
	set_pev(id, pev_maxspeed, 255.5)
}
...
public RoundEnd()
{
	set_task(4.9, "RemoveMines")
}

public RemoveMines()
{
	static ent, classname[33], id
...
SetStartAmmo(id)
{
	...
	if (g_havemine[id] >= stammo)
		return;
	else if (g_havemine[id] < stammo)
		g_havemine[id] = 1;
}
